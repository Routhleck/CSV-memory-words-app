...
bool mouseInButton(struct button* pB, MOUSEMSG m)
{
	if (pB->x < m.x && m.x <= pB->x + pB->width && pB->y < m.y && m.y <= pB->y + pB->height)
	{
		...
		else if (pB == exitMemory)
			exitMemory->color = 0xD7D7B9;
		else if (pB == review)
...
//绘制所有按钮
void drawAllButton(bool bool_start,bool bool_wordEmpty) {
...
void drawMain(bool bool_chinese, char* value0, char* value1, char* value2, int value3, int value4, char* refLastWord, char* refLastClass, char* refLastChinese, int bool_history, bool last) {
	...
	if (last) {
		...
		if (bool_history == 0) {
			...
			setbkmode(TRANSPARENT);
			settextstyle(30 * uHeight, 15 * uWidth, "黑体");
			outtextxy(730 * uWidth, 230 * uHeight, "掌握");
		}
	}
}
...
//绘制记忆进度条
void drawProcess(int ForgetTime, int MemoryTime)
{
	float forgetTime = (float)ForgetTime;
	float memoryTime = (float)MemoryTime;
	int needTime = 0;
	...
	string str = to_string(needTime);

	//红条
	setfillcolor(0x5151FF);
	setlinecolor(0x5151FF);
	fillrectangle(250 * uWidth, 550 * uHeight, 1200 * uWidth, 600 * uHeight);
	//绿条
	setfillcolor(0xA6FFA6);
	setlinecolor(0xA6FFA6);
	fillrectangle(250 * uWidth, 550 * uHeight, (250+900*(rate/0.55)) * uWidth, 600 * uHeight);

	settextcolor(BLACK);
	setbkmode(TRANSPARENT);
	settextstyle(30 * uHeight, 15 * uWidth, "等线");
	outtextxy(250 * uWidth, 560 * uHeight, "距离已掌握还需记忆次数:");
	outtextxy(650 * uWidth, 560 * uHeight, str.c_str());

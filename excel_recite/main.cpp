...
int main() {
	...
	while (1) 
	{
		...
		if (iter._Ptr != nullptr) {
			while ((((float)(iter->ReviewTime - iter->ForgetTime) / (float)(iter->ReviewTime+1)) < 0.7)&&(iter->ReviewTime != 0)) {
				iter++;
			}
		}
		temp1 = iter - tempWords.begin();
		iter = find(tempWords.begin(), tempWords.end(), 0);
		temp2 = iter - tempWords.begin();
		wordMaster = temp2 - temp1;
		...
		if (clickButton(undoMemory, m)) {
			if (bool_start == true && bool_end == false&&words[npos].ReviewTime != 0) {
				words[npos].ForgetTime--;
				if (words[npos].ForgetTime < 0)
					words[npos].ForgetTime == 0;
				words[npos].ReviewTime--;
			}
		}
